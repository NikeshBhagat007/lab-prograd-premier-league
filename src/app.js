...
//Progression 1 [START]
var createManager = (managerName,managerAge,currentTeam,trophiesWon) => {    //passed argument as to set no of argument & its sequence
let manager_info = []; 
...
//Need to know how to get that data from another JS file(data.js ) to app.js
//taken data from data.js file by me 
var players = [
  {
    name: "Luka Modric",
    age: 33,
...
var filterByPosition = (position) =>
{
  ...
  for (let i= 0;i<players.length;i++)
  {
    ...
    {
      filter_position.push(players[i]);
    }
  }
...
var filterByAward = (awardsByPlayer) =>
{
  var filter_awards = [];
  for (let i= 0;i<players.length;i++)
  {
    ...
    {
      if(players[i].awards[j].name == awardsByPlayer)
      {
        filter_awards.push(players[i]);
      }
    }
  }
  //console.log(filter_awards);
  return filter_awards;
}
